...
const props = defineProps<{ node: RollupNode; nameOf: (id: string) => string; depth?: number }>()
...
<template>
  ...
  <template v-if="open">
    <RollupRow v-for="c in node.children" :key="c.id" :node="c" :name-of="nameOf" :depth="depth + 1" />
  </template>
</template>
