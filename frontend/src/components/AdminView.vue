...
async function doLogin() {
  if (await admin.login(email.value.trim(), password.value)) {
    password.value = ''
    await load()
  }
}
...
<template>
  <div class="admin">
    <!-- Login gate -->
    <div v-if="!admin.isAdmin" class="admin-login">
      <h2>Admin sign-in</h2>
      <p class="muted">Email/password account with administrator tier.</p>
      <input v-model="email" class="sf-in" type="email" placeholder="Email" autocomplete="username" />
      <input v-model="password" class="sf-in" type="password" placeholder="Password" autocomplete="current-password"
             @keyup.enter="doLogin" />
      <button class="primary" :disabled="admin.busy" @click="doLogin">Sign in</button>
      <div v-if="admin.error" class="flash err">{{ admin.error }}</div>
    </div>

    <!-- Console -->
    <div v-else>
      <div class="admin-bar">
        <span class="muted">Admin: <b>{{ admin.username }}</b></span>
        <button class="abtn" @click="admin.logout()">Sign out</button>
      </div>
...
.admin-login { max-width: 320px; margin: 24px auto; display: flex; flex-direction: column; gap: 8px; }
.admin-login h2 { color: #f59e0b; font-size: 18px; }
